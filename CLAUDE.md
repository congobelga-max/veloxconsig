...
**Origem** says whether the lead came from a spreadsheet (**Planilha**) or was already on the server (**Base**), read from the accumulated `leads_importados` — the same registry the entry rule consults, so the column answers exactly why that row is in the Painel. Like `contato_` and the last-import mark, that key is local and changes with the list on screen, so `atualizarOrigensPainel()` copies it onto the row objects before every draw. The cell value *is* the label, so the table search filters by origin with no helper column. Two limits are in the badge's `title` rather than hidden: the registry is per browser, so a lead imported on another machine — or before the marking existed — reads as **Base**, and it is never a claim from the server. Missing `importacoes.js` degrades everyone to **Base**, matching how the entry rule degrades.
