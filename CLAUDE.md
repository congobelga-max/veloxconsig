...
`apiAtualizarContato()` writes the contact with **`PUT /clientes/:id`**, sending `contatoStatus` *and* `contatadoEmUtc`. It was a `PATCH` with only `contatoStatus` first — the request came back fine and **nothing was written to the database**, which is why the verb is PUT now. Since PUT replaces the resource, the body is the whole raw record the API itself returned (kept on `cliente.bruto`) with those two fields laid over it; a body carrying only the two would blank nome, celular and email. `contatadoEmUtc` is stamped by the browser at click time (`new Date().toISOString()`) and rendered through `formatarDataHora()`, giving back exactly the `14/08/2026 15:32` the old localStorage pair used to show.
