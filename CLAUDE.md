...
`sincronizarContatoNaApi()` sends `contatoStatus: 3` and is **not awaited** — `window.open` has to fire inside the click gesture, and waiting on the network first is exactly how a pop-up blocker eats the WhatsApp window. It runs on every contact attempt, guarded by `contato_status_<cpf>`, so contacts recorded before this feature get pushed to the server on the next approach; a failure surfaces as an error toast, because the operator needs to know the CRM and the server disagree. A client not in memory with an `idApi` is skipped with a `console.warn` — there is no route to call without it.
...
| `contato_status_<id>` | `sincronizarContatoNaApi` | `"3"` once the **server** confirmed `contatoStatus` — the guard that stops every reopened conversation from re-sending the PATCH |
